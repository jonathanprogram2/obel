...
// Choose Croq first, then Ollama as fallback
async function runLLM(messages) {
    if (!GROQ_API_KEY) {
        console.error("Missing GROQ_API_KEY — set it in your .env");
    ...
    throw new Error("Groq API call failed — check your key, model name, or internet connection.");
}
...
function clampReply(text = "", maxSentences = 3, maxChars = 125) {
    ...
    for (const s of sentences) {
        ...
        const extra = (result.length ? 1 : 0) + s.length; // +1 space

        // If adding this sentence would exceed maxChars...
        if (totalChars + extra > maxChars) {
            // ...but it's the **first** sentence, keep it anyway so we don't end up empty.
            if (result.length === 0) {
                result.push(s);
            }
            break;
    ...
    // safety: if somehow still empty but we had text, just return the cleaned text
    if (result.length === 0 && cleaned) {
        return cleaned;
    }
...
app.post("/api/assistant/chat", async (req, res) => {
    try {
        const { userId = "demo-user", message, tasks } = req.body || {};

        const rawMessage = (message || "").trim();

        // Simple "hi" / hey / "hello" detection — keep it conversational
        ...
        if (isCasualGreeting) {
            return res.json({
                reply:
                    "Hey! 👋 I'm BoBo, your AI workspace assistant. What do you need help with?",
                    suggestedTasks: [],
            });
        }
        ...
        const { boardSummaryText, deletedSummaryText } = updateTaskMemory(
            userId,
            tasks
        );
    ...
    - If the user clearly asks you to CREATE a new task or tasks (e.g. "create a task", "add a card", "make a new to-do"):
        1) First answer in natural language as usual.
        2) Then append a JSON block wrapped in <TASKS> and </TASKS> tags.
        3) The JSON MUST follow this exact shape:
    ...
    - Only include the <TASKS> JSON block when the user explicitly asks you to create/add tasks.
    ...
    `.trim();
        ...
        // extract optional <TASKS> JSON block from the model reply
        let suggestedTasks = [];
        let textPortion = replyText;
        ...
        if (taskBlockMatch) {
            ...
            // remove the whole <TASKS>...</TASKS> block from what the user sees
            textPortion = replyText.replace(taskBlockMatch[0], "").trim();
        }


        // Clean up: strip any leftover [DEV-201]-style IDs just in case
        let cleanedReply =
            (replyText || "")
                .replace(/\[[A-Za-z0-9_-]+\]/g, "") // remove [DEV-201] etc.
                .replace(/\s{2,}/g, " ")             // collapse extra space
                .trim(); 
        ...
        return res.json({
            reply: cleanedReply,
            // placeholder for future structured actions:
            suggestedTasks,
        });
    } catch (err) {
...
app.listen(PORT, () => {
    console.log(`BoBo assistant server listening on http://localhost:${PORT}`);
});